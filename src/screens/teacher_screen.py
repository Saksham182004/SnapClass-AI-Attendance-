...
def  teacher_tab_take_attendance():
    ...
    if not subjects:
        st.warning("You haven't created any Subjects yet! Please create one first.")
        return

    subject_options = {f"{s['name']} - {s['subject_code']}": s['subject_id'] for s in subjects}
    num_photos = len(st.session_state.attendance_images)

    # ── Subject picker + Add Photos ───────────────────────────────────────────
    col1, col2 = st.columns([3, 1], vertical_alignment="bottom")
    with col1:
        st.html("""<p style="color:#5865F2; font-size:0.78rem; font-weight:700;
                    letter-spacing:1px; text-transform:uppercase; margin-bottom:6px;">
                    Select Subject</p>""")
        selected_subject_label = st.pills(
            "Select Subject",
            options=list(subject_options.keys()),
            label_visibility="collapsed",
            default=list(subject_options.keys())[0]
        )
    with col2:
    ...
    if not selected_subject_label:
        st.info("Select a subject above to continue.")
        return
    ...
    ">
        ...
        <div>
            <div style="color:white; font-size:1rem; font-weight:700;">
                {num_photos} photo{'s' if num_photos!=1 else ''} queued
            </div>
            <div style="color:rgba(255,255,255,0.7); font-size:0.8rem;">
                {'Add photos above to begin scanning' if num_photos==0 else 'Ready to run AI face analysis'}
            </div>
        </div>
    </div>
    """)
    ...
    if st.session_state.attendance_images:
        st.html("""<p style="color:#64748b; font-size:0.8rem; font-weight:600;
...
def teacher_screen_register():
    ...
    teacher_username = st.text_input("Enter Username" , placeholder="name")
    teacher_name = st.text_input("Enter Your Name" , placeholder="Write your Name")
    ...
    with btnc1:
        if st.button("Register Now", icon=':material/passkey:' , shortcut='control+enter' , width='stretch'):
            success , messege = register_teacher(teacher_username,teacher_name,teacher_password,teacher_password_confirm)
            if success :
                st.success(messege)
                import time
                time.sleep(2)
